...
class fcNeuralNet:  # class for the fully connected neural net
	...
	def trainOnBatch(self,batch,batchLab,eta,batchSize,numClasses):  # train the weights and biases of the fcnn for a given batch
		for j in range(batchSize): # for each datapoint per label
			mat = np.zeros((numClasses,1))			
			dataPoint = np.matrix(batch[j].astype('float64'))
			dataPoint = dataPoint.T 
			mat[batchLab[j],0] = 1   # make expected output as one hot encoded
		...
		for i,l in enumerate(self.layers):  # for each layer l
			if i%2 == 0 and i<self.layerSize-1: # if the layer is linear 
				self.layers[i].weights = self.layers[i].weights - eta*(self.layers[i].sumDerivWeights/(batchSize) + 0.01*self.layers[i].weights) #update its weights by subtracting the learning rate multiplied by averaged derivative over the batch from each weight 
				self.layers[i].biases = self.layers[i].biases - eta*(self.layers[i].sumDerivBiases/(batchSize) + 0.01*self.layers[i].biases)#update its biases by subtracting the learning rate multiplied by averaged derivative over the batch from each bias
				self.layers[i].resetSums() # reset the accumulated sums of derivatives in each layer before training the next batch

	def trainOnData(self,trainingData,trainingLabels,trainingSize,trainIndex,eta,batchSize,numClasses): # train the fcnn on a given dataset
		...
		for i in range(numItr): # for all batches
			...
			sampleOut = np.array(trainingLabels[i*batchSize + trainIndex : (i+1)*batchSize + trainIndex])
			self.trainOnBatch(sample,sampleOut,eta,batchSize,numClasses) # train the fcnn on the batch data

	def validate(self,validationSize,numClasses,validIndex,trainingData,trainingLabels): # test the fcnn on validation data
		correct = 0
		for j in range(validationSize):	# for each datapoint per label
			dataPoint = np.matrix(trainingData[j + validIndex].astype('float64'))
			dataPoint = dataPoint.T
			op = self.forwardPass(dataPoint) # calculate output on datapoint
			l = np.argmax(op) # predicted class = argmax(softmmax output of fcnn)
			if (trainingLabels[j + validIndex] == l) : # compare predicted class with expected class
				correct += 1

		accuracy = 100*correct/(validationSize) # calculate accuracy
		return accuracy

	def test(self,testSize,numClasses,testData,testLabels): # test the fcnn on test data (same format as the function validate())
		correct = 0
		for j in range(testSize):
			dataPoint = np.matrix(testData[j].astype('float64'))
			dataPoint = dataPoint.T
